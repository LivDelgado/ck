...
public class CK {
    ...
    public CK(boolean useJars, int maxAtOnce, boolean variablesAndFields) {
        ...
        this.maxAtOnce = maxAtOnce == 0 ? getMaxPartitionBasedOnMemory() : maxAtOnce;
    }
    ...
    public void calculate(String path, CKNotifier notifier) {
        ...
        calculate(Paths.get(path), notifier,
                Stream.of(javaFiles).map(Paths::get).toArray(Path[]::new));
    }
    ...
    public void calculate(Path path, CKNotifier notifier, Path... javaFilePaths) {
        ...
        for (List<String> partition : partitions) {
            ...
            setupParser(parser, allDependencies, srcDirs);
    ...
    private void setupParser(ASTParser parser, String[] allDependencies, String[] srcDirs) {
    ...
    private int getMaxPartitionBasedOnMemory() {
        ...
        if (maxMemory >= 1500) return 300;
        if (maxMemory >= 1000) return 200;
        if (maxMemory >= 500) return 100;
        return 25;
    }
}
