...
public class LCOMNormalized implements CKASTVisitor, ClassLevelMetric {
  ...
  void internalVisit(FieldDeclaration node) {
    ...
    for (Object o : node.fragments()) {
      ...
      declaredFields.put(vdf.getName().toString(), new TreeSet<>());
    }
  }
  ...
  public void setResult(CKClassResult result) {
    ...
    // formula (13) extracted from
    // https://github.com/cqfn/jpeek/blob/master/papers/sellers96_LCOM2_LCOM3_LCOM5.pdf
    float sum = 0;
    ...
    for (String key : this.declaredFields.keySet())
      sum =
          sum
              + (((float) (numberOfMethods - this.declaredFields.get(key).size()))
                  / numberOfMethods);

    if (numberOfAttributes > 0) lcomNormalized = (((float) 1) * sum) / numberOfAttributes;
