...
public class NumberOfLogStatements implements CKASTVisitor, MethodLevelMetric, ClassLevelMetric {
   ...
   *
   * <p>Pattern based on log4j API. It also supports builder pattern (see
   * http://logging.apache.org/log4j/2.x/manual/logbuilder.html)
   *
   * <p>Limitations: 1. It does not check for EventLogs as in
   * http://logging.apache.org/log4j/2.x/manual/eventlogging.html 2. It does not support custom log
   * levels as in http://logging.apache.org/log4j/2.x/manual/customloglevels.html
   *
  ...
  void internalVisit(MethodInvocation node) {
    if (node.getParent() instanceof ExpressionStatement expr) {
      if (NumberOfLogStatements.isLogStatement(expr.toString())) qty++;
    }
  }
