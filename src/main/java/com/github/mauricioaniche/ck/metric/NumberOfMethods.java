...
public class NumberOfMethods implements CKASTVisitor, ClassLevelMetric {
  ...
  private void internalVisit(MethodDeclaration node) {
    ...
    else if (isPrivate) privateMethods++;
    else if (isProtected) protectedMethods++;
    else defaultMethods++;
    ...
    if (isAbstract) abstractMethods++;

    if (isFinal) finalMethods++;

    if (isSynchronized) synchronizedMethods++;
  }
